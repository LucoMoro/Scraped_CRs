...
//Synthetic comment -- index 0000000..b99520f
...
public class ListeningPortsTest extends TestCase {
    ...
    private static final List<String> EXCEPTION_PATTERNS = new ArrayList<String>(5);
    ...
    static {
        ...
        EXCEPTION_PATTERNS.add("[0-9A-F]{20}[0]{4}[0-9A-F]{6}7F:[0-9A-F]{4}"); // IPv4-6 Conversion
        EXCEPTION_PATTERNS.add("[0-9A-F]{20}[F]{4}[0-9A-F]{6}7F:[0-9A-F]{4}"); // IPv4-6 Conversion
    }
    ...
    private static void assertNoListeningPorts(String procFilePath, boolean isTcp) {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                assertTrue(String.format("%s should have at least %d columns of output" + fields,
                        procFilePath, expectedNumColumns), fields.length >= expectedNumColumns);

                String localAddress = fields[1];
                String state = fields[3];

                assertTrue(isAddress(localAddress));

                if (!isException(localAddress) && isPortListening(state, isTcp)) {
                    fail(String.format("Found port listening on %s in %s", localAddress,
                            procFilePath));
                }
            }
