...
final int deltaLines = newLineCount - (endLine - startLine + 1);
        if (deltaLines != 0) {
            mFirstOfChangedBlocks = firstBlock + numAddedBlocks;
            for (int i = mFirstOfChangedBlocks; i < mNumberOfBlocks; i++) {
                mBlockEndLines[i] += deltaLines;
            }
        } else {
            mFirstOfChangedBlocks = mNumberOfBlocks + 1;
}
...
return mNumberOfBlocks;
     ...
     * @hide
     */
    public int getFirstOfChangedBlocks() {
        int firstOfChangedBlocks = mFirstOfChangedBlocks;
        mFirstOfChangedBlocks = mNumberOfBlocks + 1;
        return firstOfChangedBlocks;
    }
...
private int mNumberOfBlocks;
    ...
    private int mFirstOfChangedBlocks;
...
final int numberOfBlocks = dynamicLayout.getNumberOfBlocks();
            final int firstOfChangedBlocks = dynamicLayout.getFirstOfChangedBlocks();
...
if (blockIsInvalid) blockDisplayList.invalidate();
                ...
                if (i >= firstOfChangedBlocks || !blockDisplayList.isValid()) {
