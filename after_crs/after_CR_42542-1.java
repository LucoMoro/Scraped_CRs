...
private static final int MOTIONLESS_TIME = 100;
    // needed to avoid accelerating flings when not intended by the user
    private static final int MAX_FLING_ACCELERATION_TIME = 500;
...
mPrivateHandler.removeMessages(DRAG_HELD_MOTIONLESS);
                        // if the user stops the scrolling we shouldn't accelerate
                        // the fling when starting to scroll again
                        if (eventTime - event.getDownTime() >= MAX_FLING_ACCELERATION_TIME) {
                            abortAnimation();
                        }
