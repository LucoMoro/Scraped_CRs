...
//Synthetic comment -- index 1fe6d97..825a5c6 100644
...
//Synthetic comment -- index f3da39c..e922753 100644
...
if (libMap != null) {
//Synthetic comment -- @@ -121,6 +124,44 @@
return mName;
}
...
//Synthetic comment -- index c0dcaa7..96bb297 100644
...
/**
    ...
    * <p/>parameter should be a string returned from getAbiList() or "arm"
    */
    public void setProcessorType(String processorType);
...
//Synthetic comment -- index 6aeeade..043ce7f 100644
...
SdkConstants.FN_DX_JAR);
	mAbiType = SdkConstants.ARM_PROCESSOR;
    }
    ...
    public void setProcessorType(String processorType) {
         mAbiType = processorType;
         String processorList[] = getAbiList();
    ...
    public String getPathForAbi() {
        if(getAbiList().length==0) {
            ...
            return "";
               ...
               return mAbiType + File.separator;
        }
    }
...
//Synthetic comment -- index 37265b1..d5a0766 100644
...
//Synthetic comment -- index eba8e07..d2f0235 100644
...
// writes the userdata.img in it.
            String imagePath = target.getPath(IAndroidTarget.IMAGES) + target.getPathForAbi();

File userdataSrc = new File(imagePath, USERDATA_IMG);

if (userdataSrc.exists() == false && target.isPlatform() == false) {
                imagePath =
                    target.getParent().getPath(IAndroidTarget.IMAGES) + target.getPathForAbi();
...
//Synthetic comment -- @@ -861,7 +910,7 @@
*/
private String getImageRelativePath(IAndroidTarget target)
throws InvalidTargetPathException {
        String imageFullPath = target.getPath(IAndroidTarget.IMAGES) + target.getPathForAbi();
...
//Synthetic comment -- index 4411034..c2e5b96 100644
...
error = "A target must be selected in order to create an AVD.";
        ...
        if (hasAvdName && error == null && mTargetCombo.getSelectionIndex() > 0) {
            ...
            String targetName = mTargetCombo.getItem(index);
            IAndroidTarget target = mCurrentTargets.get(targetName);
            if(target.getAbiList().length > 0 && mProcessorTypeCombo.getSelectionIndex() < 0) {
