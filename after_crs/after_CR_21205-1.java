...
//Synthetic comment -- @@ -172,6 +177,10 @@
        ...
        mNameWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(IMAGE_EMPTY, //$NON-NLS-1$
                mShell.getDisplay()));
...
//Synthetic comment -- @@ -202,6 +210,10 @@
        ...
        mTagWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(IMAGE_EMPTY, //$NON-NLS-1$
                mShell.getDisplay()));
...
mPidWarning = new Label(main, SWT.NONE);
        mPidWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(IMAGE_EMPTY, //$NON-NLS-1$
mShell.getDisplay()));
...
mPidWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_WARNING,
mShell.getDisplay()));
                mPidWarning.setToolTipText("Allow only digits"); //$NON-NLS-1$
                result = false;
} else {
mPidWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_EMPTY,
mShell.getDisplay()));
        ...
        if (mTag != null) {
            if (mTag.matches(".*[:|].*") == true) { //$NON-NLS-1$
                ...
                mTagWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_WARNING,
                        mShell.getDisplay()));
                mTagWarning.setToolTipText("Not allow characters | or :"); //$NON-NLS-1$
                result = false;
            } else {
                mTagWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_EMPTY,
        ...
        if (mName != null && mName.length() > 0) {
            if (mName.matches(".*[:|].*") == true) { //$NON-NLS-1$
                ...
                mNameWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_WARNING,
                        mShell.getDisplay()));
                mNameWarning.setToolTipText("Not allow characters | or :"); //$NON-NLS-1$
                result = false;
            } else {
                mNameWarning.setImage(ImageLoader.getDdmUiLibLoader().loadImage(
                        IMAGE_EMPTY,
        ...
        if (result) mOkButton.setEnabled(true);
}
}
