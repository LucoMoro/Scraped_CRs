...
// Recenter the tab display if we're at a new (scrollable) size.
            setTabHostSelection(mSelectedTabIndex);
}
}
...
ViewGroup.LayoutParams.MATCH_PARENT));
        setTabHostSelection(mTabSpinner.getSelectedItemPosition());
return false;
}
...
mSelectedTabIndex = position;
        if (isCollapsed()) {
            ...
            setTabHostSelection(position);
        }
    }

    private void setTabHostSelection(int position) {
