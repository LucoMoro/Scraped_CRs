...
AdtPrefs.getPrefs().setSdkLocation(bundledSdk);
                ...
                // since we already have an SDK, just show the usage data opt-in page
                showWelcomeWizard(false, true);
}
}

if (isFirstTime()) {
            showWelcomeWizard(true, true);
...
// run ADT which is why the wizard was opened. No need to ask again.
        if (mShowUsagePage && !mStore.isPingOptIn()) {
