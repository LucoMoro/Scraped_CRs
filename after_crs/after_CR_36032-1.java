...
SdkRepoConstants.NODE_PREVIEW,
        ...
        mPreviewVersion = new FullRevision(
                super.getRevision().getMajor(), minorRevision, microRevision, preview);
}
...
super.saveProperties(props);
        ...
        // The major revision is super.getRevision(), already handled by Package.
        assert mPreviewVersion.getMajor() == super.getRevision().getMajor();
...
sb.append("|r:");                                                       //$NON-NLS-1$
        sb.append(String.format("%1$s", getRevision().toString()));             //$NON-NLS-1$
...
// only one platform-tool package so any platform-tool package is the same item.
        // however previews can only match previews.
        return (pkg instanceof PlatformToolPackage) &&
            ((PlatformToolPackage) pkg).getRevision().isPreview() == getRevision().isPreview();
}
...
public boolean sameItemAs(Package pkg) {
        ...
        return (pkg instanceof ToolPackage) &&
            ((ToolPackage) pkg).getRevision().isPreview() == getRevision().isPreview();
}
...
return archives;
    ...
    private float getRevisionRank(FullRevision rev) {
        return rev.getMajor() +
               rev.getMinor() / 100.f +
               rev.getMicro() / 10000.f +
               rev.getPreview() / 1000000.f;
    }
...
for (Package p : localPkgs) {
                    float rev = getRevisionRank(p.getRevision());
int api = 0;
...
getTree(m, false /*displaySortByApi*/));
    ...
    public void testPreviewUpdateInstalledRelease() {
        ...
        assertEquals(
                ...
                "PkgCategoryApi <API=TOOLS-PREVIEW, label=Tools (Beta Channel), #items=2>\n" +
                "-- <INSTALLED, pkg:Android SDK Tools, revision 4 rc1>\n" +
                "-- <INSTALLED, pkg:Android SDK Platform-tools, revision 4 rc1>\n" +
    ...
    public void __testPreviewUpdateInstalledPreview() {
        // Test: Local preview Tools 3.0.1rc1 installed, server has both a release 3.0.1 available
        // and a Tools Preview 3.0.1 rc2 available.
        // => v3.0.1rc2 does not update 3.0.1, instead it updates v3.0.1rc1.
        ...
        m.updateStart();
        m.updateSourcePackages(true /*sortByApi*/, null /*locals*/, new Package[] {
                new MockToolPackage(null, new FullRevision(3, 0, 1, 1), 4),       //  T 3.0.1rc1
