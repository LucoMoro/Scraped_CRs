...
//Synthetic comment -- index 50fc496..0e23266 100644

//Synthetic comment -- @@ -108,15 +108,20 @@
mLog = logger;
mActions = actions;
...
//Synthetic comment -- @@ -580,23 +585,20 @@
...
/**
//Synthetic comment -- @@ -887,4 +892,58 @@
...
mLog.error(null, format, args);
     ...
     */
    private String reflowLine(String line) {
        final int maxLen = 78;
        ...
        int prefixSize = Math.max(4, line.indexOf(':') + 1);
        ...
        while (n > 0) {
            ...
            // Line is longer than the max length, find the first character before the
            // end where we want to break the line.
            int c = maxLen;
            while (c > 0 && line.charAt(c) != ' ') {
                c--;
            }
            if (c == 0) {
                // Hmm we found no whitespace separator. This should generally not occur.
                c = maxLen;
            }

            output += line.substring(0, c);
            output += '\n';
            line = prefix + line.substring(c);
            n -= c;
        }
