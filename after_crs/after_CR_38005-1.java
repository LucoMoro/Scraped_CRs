...
//Synthetic comment -- index 03773b0..bccf802 100644
...
//Synthetic comment -- @@ -43,6 +52,8 @@
private final static String sDevicesFilename = "devices.xml";
...
//Synthetic comment -- @@ -113,6 +124,75 @@
return mUserDevices;
}
    ...
    public static Map<String, String> getHardwareProperties(State s) {
        ...
        props.put("hw.sdCard", getBooleanVal(hw.getRemovableStorage().size() > 0));
        int density;
        switch(hw.getScreen().getPixelDensity()) {
        case LOW:
            density = 120;
            break;
        case MEDIUM:
        ...
        props.put("hw.lcd.density", Integer.toString(density));
...
private Collection<? extends Device> loadDevices(File deviceXml) {
try {
...
//Synthetic comment -- index 4e54da0..f21fa92 100644
...
//Synthetic comment -- @@ -303,10 +318,32 @@
...
//
//Synthetic comment -- @@ -823,6 +861,66 @@
            ...
            if (mAbiTypeCombo.isEnabled()) {
                ...
                for (Abi abi : abis) {
                    for (int i = 0; i < mAbiTypeCombo.getItemCount(); i++) {
                        if (mAbiTypeCombo.getItem(i).equals(AvdInfo.getPrettyAbiType(abi.toString()))){
