...
DisplayMetrics metrics = mWebView.getContext().getResources().getDisplayMetrics();
        // relate the density to the default value dimension
        int dimension = (int)(Math.max(metrics.widthPixels, metrics.heightPixels) / 
...
WidgetTestUtils.assertScaledPixels(285, mRatingBar.getMeasuredWidth(), getActivity());
        	...
        	// relate the density to the default value '285'
            assertEquals((int)(285*getActivity().getResources().getDisplayMetrics().density), mRatingBar.getMeasuredWidth());
}
