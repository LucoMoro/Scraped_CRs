/*Skip the test case about SCREENLAYOUT_LONG if the device has the navigation bar

Occured Condition
  1. The device has WXGA (1280x800) display.
  2. The device has navigation bar.
...
I modified so that if the device has the navigation bar, this test is skipped.*/
...
//Synthetic comment -- index d4c3611..52337ec 100644
...
+ " for orientation " + ORIENTATIONS[i], expectedSize, actualSize);
            if (!navigationBar) {
                assertEquals("Expected screen long value of " + expectedLong + " but got "
                    + actualLong + " for orientation " + ORIENTATIONS[i], expectedLong, actualLong);
            }
tearDown();
}
}
//Synthetic comment -- @@ -80,6 +86,11 @@
return screenLayout;
}
