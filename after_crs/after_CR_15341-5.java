...
//Synthetic comment -- index 0000000..685f7a8

//Synthetic comment -- @@ -0,0 +1,650 @@
/*
 *
...
public class ApkBuilder {
    ...
    public final static class JarStatusImpl implements JarStatus {
        ...
        public boolean nativeLibsConflict;

        public JarStatusImpl(List<String> libs, boolean nativeLibsConflict) {
            this.libs = libs;
            this.nativeLibsConflict = nativeLibsConflict;
        }
    }
     ...
     */
    public JarStatus addJar(File jarFile) throws ApkCreationException, SealedApkException,
        ...
        try {
            ...
            throw (DuplicateApkFileException)e;
        }
    }
    ...
    public void addSourceFolder(File sourceFolder) throws ApkCreationException, SealedApkException,
        ...
        if (sourceFolder.isDirectory()) {
            try {
                ...
                if (e instanceof DuplicateApkFileException) {
                    throw (DuplicateApkFileException)e;
                } else {
                    throw new ApkCreationException(e);
                }
            ...
            if (sourceFolder.exists()) {
                throw new ApkCreationException(sourceFolder.getAbsolutePath() + " is not a folder!");
            } else {
                throw new ApkCreationException(sourceFolder.getAbsolutePath() + " does not exist!");
            }
        }
    }
    ...
    private void processFileForResource(File file, String path)
        ...
        if (file.isDirectory()) {
            ...
            if (checkFileForPackaging(file.getName())) {
                ...
                File duplicate = checkFileForDuplicate(path);
                if (duplicate == null) {
                    verbosePrintln("%1$s => %2$s", file, path);
                    mAddedFiles.put(path, file);
                    mBuilder.writeFile(file, path);
                } else {
                    throw new DuplicateApkFileException(path, duplicate, file);
                }
            }
        }
    }
