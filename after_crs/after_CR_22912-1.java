...
//Synthetic comment -- index d304303..08ae107 100644
...
//Synthetic comment -- @@ -1045,6 +1051,27 @@
Collections.sort(matches, new PhoneConfigComparator());
        ...
        // look up its configuration and pick the first match that matches the current
        // configuration. In other words, "preserve" the current config when we're looking
        // for a new config. (We do this after sorting to make sure that we pick the best
        // match, according to the above sort, if there are multiple matching configurations.)
        ...
        if (activeEditor instanceof LayoutEditor) {
            ...
            if (configuration != null) {
                for (ConfigMatch match : matches) {
                    if (configuration.isMatchFor(match.testConfig)) {
                        return match;
                    }
                }
            }
...
//Synthetic comment -- @@ -1282,16 +1309,16 @@
