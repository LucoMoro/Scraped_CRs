...
//Synthetic comment -- index d304303..660e6bb 100644

//Synthetic comment -- @@ -2400,6 +2400,27 @@
}
}
                        ...
                        if (version.getCodename() != null && mTargetList != null) {
                            ...
                            for (IAndroidTarget t : mTargetList) {
                                if (t.getVersion().getApiLevel() == targetApiLevel) {
                                    // No need to check that the vendor fields are equal
                                    // etc since the target list here is the filtered
                                    // version which contains only valid rendering targets,
                                    // not all android targets
                                    target = t;
                                    break;
                                }
                            }
