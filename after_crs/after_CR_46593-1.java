/*Change DeviceManager to make more stateful.

This fixes an optimization issue where it was reparsing
the same device list from disk at every call to
getDevices() and getDeviceStatus(). Each time it would
clear its list and rebuild it.

This change the manager so that callers create one instance
that contains all the lists and there no static data shared
...
SdkConstants.OS_SDK_DOCS_FOLDER);
        ...
        mDeviceManager = DeviceManager.getInstance(manager.getLocation(), AdtPlugin.getDefault());
...
assertEquals("@style/Theme", configuration.getTheme());
        ...
        DeviceManager deviceManager =
            DeviceManager.getInstance(null /*osSdkPath*/, new StdLogger(StdLogger.Level.VERBOSE));
...
//Synthetic comment -- index 3662c26..87ff622 100644
...
MISSING;
     ...
     */
    public static DeviceManager getInstance(@Nullable String osSdkPath, @NonNull ILogger log) {
...
//Synthetic comment -- @@ -190,53 +230,51 @@
...
//Synthetic comment -- @@ -244,11 +282,12 @@
...
//Synthetic comment -- @@ -274,34 +314,44 @@
}
}
}
        return false;
...
int deviceHash = Integer.parseInt(hash);
                DeviceManager devMan = DeviceManager.getInstance(mSdkManager.getLocation(), log);
...
mOwnUpdaterData = true;
        mDeviceManager = DeviceManager.getInstance(osSdkRoot, sdkLog);
}
...
mOwnUpdaterData = false;
        mDeviceManager = DeviceManager.getInstance(mUpdaterData.getOsSdkRoot(),
...
//Synthetic comment -- index e5d94fb..c350317 100755
...
if (sdkManager != null && location != null) {
            DeviceManager deviceManager = DeviceManager.getInstance(location, mSdkLog);
...
Map<String, String> properties = new HashMap<String, String>(avdInfo.getProperties());
            ...
            DeviceManager devMan = DeviceManager.getInstance(mOsSdkPath, mSdkLog);
...
mSettingsController = settingsController;
        mDeviceManager = DeviceManager.getInstance(mSdkLocation, sdkLog);
if (mAvd == null) {
...
//Synthetic comment -- index 82972cf..8ef9d25 100644
