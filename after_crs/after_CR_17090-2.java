...
//Synthetic comment -- index f5ad935..3258c63 100644
...
//Synthetic comment -- @@ -300,46 +290,116 @@
...
/**
//Synthetic comment -- @@ -347,19 +407,19 @@
*
...
//Synthetic comment -- @@ -378,20 +438,20 @@
...
//Synthetic comment -- @@ -657,12 +719,7 @@
...
/**
//Synthetic comment -- @@ -1261,21 +1320,6 @@
...
*/
//Synthetic comment -- @@ -1635,13 +1679,6 @@
}
}
...
//Synthetic comment -- index f36d9d5..94ea40b 100755
...
//Synthetic comment -- @@ -337,10 +387,28 @@
s = s / 2;
}
    ...
    private void rescaleToReal(boolean real) {
        if (real) {
            // simply recompute. The scale is set on each redraw in this mode so that change in
            // device use the dpi of the device.
            recomputeLayout();
        } else {
...
@Override
public void dispose() {
//Synthetic comment -- @@ -449,10 +517,6 @@
recomputeLayout();
}
...
mEditedFile.getName(), mConfigComposite.getCurrentConfig());
//Synthetic comment -- @@ -812,10 +876,25 @@
}
...
if (currentSdk != null) {
//Synthetic comment -- @@ -980,6 +1059,17 @@
...
if (bridge.bridge != null) { // bridge can never be null.
                    ...
                    if (mZoomRealSizeButton.getSelection()) {
                        ...
                        mCanvasViewer.getCanvas().setScale(monitor / dpi, false /*redraw*/);
                    }

renderWithBridge(iProject, model, bridge);
} else {
// SDK is loaded but not the layout library!
//Synthetic comment -- @@ -1098,7 +1188,7 @@
...
theme, isProjectTheme,
configuredProjectRes, frameworkResources, mProjectCallback,
//Synthetic comment -- @@ -1115,6 +1205,7 @@
displayError(result.getErrorMessage());

} else {
            System.out.println("BOX: " + result.getRootView().getRight() + " x " + result.getRootView().getBottom());
...
//Synthetic comment -- index c143c35..7811403 100755
...
/**
//Synthetic comment -- @@ -812,11 +814,11 @@
}

public int scale(int canwasW) {
            return (int)Math.round(mScale * canwasW);
}

public int inverseTranslate(int screenX) {
            return (int) Math.round((double)(screenX - IMAGE_MARGIN + mTranslate) / mScale);
}
}
