...
//Synthetic comment -- index 05f0adf..73e1c55 100755
...
/**
//Synthetic comment -- @@ -160,4 +181,154 @@
...
//Synthetic comment -- index 0000000..f9566b9

//Synthetic comment -- @@ -0,0 +1,359 @@
/*
...
public class AttributeInfoTest extends TestCase {
    ...
    public void testIsValid() throws Exception {
        ...
        checkDir(new File(git, "packages" + File.separator + "apps"), false, attributeMap);
    }

    private void checkDir(File dir, boolean isResourceDir,
            Map<String, AttributeInfo> map) throws IOException {
        assertTrue(dir.isDirectory());
        File[] list = dir.listFiles();
        if (list != null) {
            for (File file : list) {
                if (isResourceDir && file.isFile() && file.getPath().endsWith(DOT_XML)) {
                    checkXmlFile(file, map);
                } else if (file.isDirectory()) {
                    checkDir(file, isResourceDir || file.getName().equals("res"), map);
                }
            }
        }
    }

    private void checkXmlFile(File file, Map<String, AttributeInfo> map) throws IOException {
        String xml = Files.toString(file, Charsets.UTF_8);
        if (xml != null) {
            //Document doc = DomUtilities.parseStructuredDocument(xml);
            Document doc = DomUtilities.parseDocument(xml, false);
            if (doc != null && doc.getDocumentElement() != null) {
                checkElement(file, doc.getDocumentElement(), map);
            }
        }
    }
        ...
        for (int i = 0, n = attributes.getLength(); i < n; i++) {
            ...
            if (ANDROID_URI.equals(uri)) {
                ...
                if ((value.startsWith("@") || value.startsWith("?")) &&
                    ...
                    System.out.println("Warning: Found resource reference for attribute "
                        + name + " which doesn't allow references ("
                            + info.getFormats() + "): value was " + value);
                }
            }
        }
    }
