...
new file mode 100644
//Synthetic comment -- index 0000000..745b236

//Synthetic comment -- @@ -0,0 +1,393 @@
/*
 * Copyright (C) 2013 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
public class DdmViewDebugDevice extends AbstractHvDevice implements IDeviceChangeListener {
    ...
    private static class DumpViewHierarchyHandler extends ViewDumpHandler {
        public DumpViewHierarchyHandler() {
            super(HandleViewDebug.CHUNK_VUDP);
        }
    ...
    private static class CaptureViewHandler extends ViewDumpHandler {
        public CaptureViewHandler(int type) {
            super(type);
        }
    ...
    private static class CaptureLayersHandler extends ViewDumpHandler {
        ...
        public CaptureLayersHandler() {
            super(HandleViewDebug.CHUNK_VUCL);
        }
    ...
    public void loadProfileData(Window window, ViewNode viewNode) {
        ...
        String viewRoot = window.getTitle();
        CaptureViewHandler handler = new CaptureViewHandler(HandleViewDebug.CHUNK_VUPR);
        try {
    ...
    public Image loadCapture(Window window, ViewNode viewNode) {
        ...
        String viewRoot = window.getTitle();
        CaptureViewHandler handler = new CaptureViewHandler(HandleViewDebug.CHUNK_VUCV);
