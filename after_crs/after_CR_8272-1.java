...
private static final byte WAP_CO_MIME_PORT = (byte)0xb2;
    ...
    static final int WAP_PDU_SHORT_LENGTH_MAX = 30;

    static final int WAP_PDU_LENGTH_QUOTE = 31;
...
int headerStartIndex = index;
        ...
        Wap230WspContentType contentType = Wap230WspContentType.extractContentType(pdu, index);
...
intent.putExtra("data", data);
        intent.putExtra("contentTypeParameters", contentType.getParameters());
...
/**
 * Representation and parser for the wap-230-wsp-20010705-a (Section 8.4.2.24)
 * Content-Type header. Any of the well-known Content Type numbers (see
 * http://www.openmobilealliance.org/Tech/omna/omna-wsp-content-type.aspx) are
 * parsed and available as strings.
 */
public class Wap230WspContentType {
    ...
    private static final int NON_INTEGER = -1;
    ...
    private Wap230WspContentType(byte[] source, int startIndex) {
        this.mSource = source;
        this.mIndex = startIndex;

        byte firstByte = mSource[mIndex++];

        if ((firstByte & 0x80) == 0x00 && firstByte <= 31) {
            // Content-general-form
            int valueLength = 0;
            if (firstByte == SMSDispatcher.WAP_PDU_LENGTH_QUOTE) {
                valueLength = readUintvar();
            } else {
                valueLength = firstByte;
            }

            int wellKnownValue = readIntegerValue();
            if (wellKnownValue == NON_INTEGER) {
                mMimeType = readString();
            } else {
                mMimeType = WELL_KNOWN_MIME_TYPES.get(wellKnownValue);
            }

            // parse any parameters if required - valueLength includes
            // the length of any parameters
            while (startIndex + valueLength > mIndex) {
                parseNextParameter();
            }
            ...
            // Constrained-media
            if ((firstByte > 31)) {
                mIndex--; // first byte is part of the string, so move back before reading
                mMimeType = readString();
            } else {
                mMimeType = WELL_KNOWN_MIME_TYPES.get((int)firstByte & 0x7F);
            }
        }
    }

    /**
     * Parses the next Content-Type parameter from the header and inserts it
     * into the parameters map
     */
    private void parseNextParameter() {

        byte nextByte = mSource[mIndex];
        String value = null;
        String param = null;
        if ((nextByte & 0x80) == 0x00 && nextByte > 31) { // untyped
            param = readString();
        } else { // typed
            int wellKnownParameterValue = readIntegerValue();
            ...
            if (wellKnownParameterValue == Q_VALUE) {
                value = String.valueOf(readUintvar());
                mParameters.put(param, value);
                return;
            }
        }

        int intValue = readIntegerValue();
    ...
    /**
     * Parse the ContentType headers from an array of bytes, as specified in
     * wap-230-wsp-20010705-a (Section 8.4.2.24)
     * 
     * @param source the byte array of the underlying pdu
     * @param startIndex the start of the ContentType header within this array
     * @return a new Wap230WspContentType object representing the
     *         ContentTypeHeader
     */
    public static Wap230WspContentType extractContentType(byte[] source, int startIndex) {
        return new Wap230WspContentType(source, startIndex);
    }

    /**
     * Returns the MimeType associated with this Content-Type. Any optional
     * parameters in the content header are not returned by this function,
     * getParameters() must be used instead.
     * 
     * @return String the MIME type of the Content Type header
     */
    ...
    /**
     * Returns the Parameter map associated with this Content-Type
     * 
     * @return Map<String, String> Parameters, where parameter name is the key
     */
    ...
    /**
     * Read a NULL terminated string from the current position in the source
     * buffer.
     */
    private String readString() {
        int start = mIndex;     //create local copies for efficiency
        int index = start;
        byte[] source = mSource;
        ...
        mIndex = index;         //write the current position back to the field
        return new String(mSource, start, mIndex - start - 1);
    }

    /**
     * Read a UintVar variable length integer (wap-230-wsp-20010705-a, section
     * 8.1.2) from the current position in source buffer.
     * 
     * @return a uintvar
     */
    private int readUintvar() {
        ...
        do {
            temp = mSource[mIndex++];
            uintlength = uintlength << 7;
            uintlength |= temp & 0x7F;
        } while ((temp & 0x80) != 0);
    ...
    private int readIntegerValue() {
        ...
        if ((firstByte & 0x80) == 0x80) {
            // Is it a short-integer (0-127 with MSB set)
            return (int)mSource[mIndex++] & 0x7F;
        } else if (firstByte <= SMSDispatcher.WAP_PDU_SHORT_LENGTH_MAX) {
            byte length = mSource[mIndex++];
            // assume length no larger than 4 bytes so fits in an int
            if (length > 4) length = 4;
            int value = 0;
            for (byte i = 0; i < length; i++) {
                value = value << 8 | mSource[mIndex++];
            }
            return value;
        }
        return -1; // Unable to read an integer value from current position
    }
}
...
public class Wap230WspContentTypeTest extends TestCase {
	...
	public void testWellKnownShortIntegerMimeTypeValues() {
		...
		for(int value : Wap230WspContentTypeTest.WELL_KNOWN_MIME_TYPES.keySet()) {
			if (value < 128) {
				String mimeType = Wap230WspContentType.extractContentType(HexDump.toByteArray((byte) (value | 0x80)), 0).getMimeType();
	...
	public void testWellKnownLongIntegerMimeTypeValues() {
	        ...
	        for(int value : Wap230WspContentTypeTest.WELL_KNOWN_MIME_TYPES.keySet()) {
	            if (value > 0x0200) {
	                ...
	                String mimeType = Wap230WspContentType.extractContentType(stuff, 0).getMimeType();
	                assertEquals(Wap230WspContentTypeTest.WELL_KNOWN_MIME_TYPES.get(value), mimeType);
	                numberFound++;
	            }
	        }
	...
	public void testConstrainedMediaExtensionMedia() throws Exception {
		...
		String mimeType = Wap230WspContentType.extractContentType(out.toByteArray(), 0).getMimeType();
	...
	public void testGeneralFormShortLengthExtensionMedia() throws Exception {
		...
		String testType = "123456789012345678901234567890";
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		out.write(30);
		out.write(testType.getBytes("US-ASCII"));
		out.write(0x00);
		...
		String mimeType = Wap230WspContentType.extractContentType(out.toByteArray(), 0).getMimeType();
		assertEquals("123456789012345678901234567890", mimeType);
	...
	public void testTypedParamWellKnownShortIntegerNoValue() throws Exception {
        ...
        Wap230WspContentType contentType = Wap230WspContentType.extractContentType(out
                .toByteArray(), 0);
        assertEquals("application/vnd.wap.rollover-certificate", contentType.getMimeType());
