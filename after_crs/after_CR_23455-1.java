    ...
    public void updatePreview(ResourceType type, String resource) {
        ...
        if (showPreview) {
            ...
            if (type == ResourceType.COLOR) {
                ...
                ResourceResolver resources = mEditor.getResourceResolver();
                ResourceValue drawableResourceValue = resources.findResValue(resource, false);

                RenderService renderService = RenderService.create(mEditor).setRenderingMode(
                        RenderingMode.FULL_EXPAND).setSize(WIDTH, HEIGHT);
                image = renderService.renderDrawable(drawableResourceValue);
            }
            Display display = mEditor.getSite().getShell().getDisplay();
            if (image != null) {
                mPreviewImageControl.setImage(SwtUtils.convertToSwt(display, image, false, -1));
            } else {
