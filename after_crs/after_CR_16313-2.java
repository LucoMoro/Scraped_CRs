        ...
        public void handleEvent(Event e) {
            synchronized (this) {
                if (tree != null && viewport != null) {
                    ...
                    // Snap the viewport to bounds.
                    if (viewport.width > tree.bounds.width) {
                        viewport.x = Math.min(viewport.x, tree.bounds.x);
                        viewport.x =
                                Math.max(viewport.x, tree.bounds.x + tree.bounds.width
                                        - viewport.width);
                    } else {
                        viewport.x = Math.max(viewport.x, tree.bounds.x);
                        viewport.x =
                                Math.min(viewport.x, tree.bounds.x + tree.bounds.width
                                        - viewport.width);
                    }
                    if (viewport.height > tree.bounds.height) {
                        viewport.y = Math.min(viewport.y, tree.bounds.y);
                        viewport.y =
                                Math.max(viewport.y, tree.bounds.y + tree.bounds.height
                                        - viewport.height);
                    } else {
                        viewport.y = Math.max(viewport.y, tree.bounds.y);
                        viewport.y =
                                Math.min(viewport.y, tree.bounds.y + tree.bounds.height
                                        - viewport.height);
                    }
                }
            }
