...
super.commit(onSave);
    ...
    public void setFocus() {
        ...
        /*DEBUG*/ s.setBackground(s.getDisplay().getSystemColor(SWT.COLOR_RED)); /*DEBUG*/

        Control[] cs = s.getChildren();
        for (Control c : cs) {
            AdtPlugin.printErrorToConsole("child", c.toString());

            c.addFocusListener(new FocusListener() {
                public void focusGained(FocusEvent e) {
                    AdtPlugin.printErrorToConsole("focus gained", e.widget.toString());
                    changeFocus(getTable());
                }

                private boolean changeFocus(Composite parent) {
                    if (parent == null) {
                        return false;
                    }
                    for (Control c : parent.getChildren()) {
                        if (!c.isDisposed() && c.isEnabled() && c instanceof Text) {
                            if ((c.getStyle() & SWT.READ_ONLY) == 0) {
                                if (c.setFocus()) {
                                    /*DEBUG*/ c.setBackground(c.getDisplay().getSystemColor(SWT.COLOR_GREEN)); /*DEBUG*/
                                    return true;
                                }
                            }
                        } else if (c instanceof Composite) {
                            if (changeFocus((Composite) c)) {
                                return true;
                            }
                        }
                    }
                ...
                public void focusLost(FocusEvent e) {
                    AdtPlugin.printErrorToConsole("focus lost", e.widget.toString());
