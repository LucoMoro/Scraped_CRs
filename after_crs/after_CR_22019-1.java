...
// timeout could be expected or unexpected. The caller will decide.
            Log.e(TAG, "waitForFocusDone: timeout");
}
mFocusDone.close();
return result;
...
double fps = 1000.0 / avgInterval;
                ...
                if ( 1 == mFrames.size() ) {
                    ...
                    mFPS = mLastFPS + (fps - mLastFPS)/mFrames.size();
                }

                assertTrue("Actual fps (" + mFPS  + ") should be larger than " +
"min fps (" + mMinFps + ")",
                           mFPS  >= mMinFps * (1 - fpsMargin));
                assertTrue("Actual fps (" + mFPS  + ") should be smaller than " +
"max fps (" + mMaxFps + ")",
                           mFPS  <= mMaxFps * (1 + fpsMargin));
}
