...
public static BufferedImage convertToAwt(Image swtImage) {
        ...
        PaletteData swtPal = swtData.palette;
        if (swtPal.isDirect) {
            PaletteData awtPal = getAwtPaletteData(awtImage.getType());

            if (swtPal.equals(awtPal)) {
                ...
                int sr = -awtPal.redShift   + swtPal.redShift;
                int sg = -awtPal.greenShift + swtPal.greenShift;
                int sb = -awtPal.blueShift  + swtPal.blueShift;

                for (int y = 0; y < swtData.height; y++) {
                    for (int x = 0; x < swtData.width; x++) {
                      int pixel = swtData.getPixel(x, y);

                      int r = pixel & swtPal.redMask;
                      int g = pixel & swtPal.greenMask;
                      int b = pixel & swtPal.blueMask;
                      r = (sr < 0) ? r >>> -sr : r << sr;
                      g = (sg < 0) ? g >>> -sg : g << sg;
                      b = (sb < 0) ? b >>> -sb : b << sb;
