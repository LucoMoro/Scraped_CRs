...
//Synthetic comment -- index 49603bc..18d5303 100644
...
//Synthetic comment -- @@ -351,8 +358,13 @@
return sb.toString();
}

    public String getTestDownloadUrl(int numBytes) {
...
//Synthetic comment -- @@ -607,6 +618,34 @@
return null;
}
    ...
    private HttpResponse createTestDownloadResponse(Uri uri) throws IOException {
        ...
        response.setEntity(createFileEntity(numBytes));
        return response;
    }

    private FileEntity createFileEntity(int numBytes) throws IOException {
        String storageState = Environment.getExternalStorageState();
        if (Environment.MEDIA_MOUNTED.equalsIgnoreCase(storageState)) {
            File storageDir = Environment.getExternalStorageDirectory();
            File file = File.createTempFile("cts-test-download-", ".bin", storageDir);
...
//Synthetic comment -- index fbe57c4..830e56f 100644
...
Cursor cursor = null;
try {
//Synthetic comment -- @@ -116,13 +164,17 @@
}

private Uri getGoodUrl() {
        return Uri.parse(mWebServer.getTestDownloadUrl(1337));
}
...
return Uri.parse(mWebServer.getBaseUri() + "/nosuchurl");
    ...
    private Uri getMinimumDownloadUrl() {
        return Uri.parse(mWebServer.getTestDownloadUrl(MINIMUM_DOWNLOAD_BYTES));
    }
