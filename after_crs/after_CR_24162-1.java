...
task.createArg().setValue(mAssets);
        ...
        // add other assets coming from library project
        Object libAssetRef = taskProject.getReference(AntConstants.PROP_PROJECT_LIBS_ASSET_REF);
        if (libAssetRef instanceof Path) {
            for (String path : ((Path)libAssetRef).list()) {
...
public static final String PROP_PROJECT_LIBS_RES_REF = "project.libraries.res";
    /** ant property ref to the list of asset folder for the project libraries */
    public static final String PROP_PROJECT_LIBS_ASSET_REF = "project.libraries.asset";
...
Path resPath = new Path(antProject);
        Path assetPath = new Path(antProject);
...
element.setPath(path + "/" + SdkConstants.FD_RESOURCES);
                ...
                // get the assets path. Always $PROJECT/assets
                element = assetPath.createPathElement();
                element.setPath(path + "/" + SdkConstants.FD_ASSETS);
...
antProject.addReference(AntConstants.PROP_PROJECT_LIBS_RES_REF, resPath);
            antProject.addReference(AntConstants.PROP_PROJECT_LIBS_ASSET_REF, assetPath);
