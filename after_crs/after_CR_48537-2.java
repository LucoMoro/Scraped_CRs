...
File valuesFolder = new File(rootFolder, folderName);
                if (!valuesFolder.mkdirs()) {
                    throw new IOException("Failed to create directory: " + valuesFolder);
                }
...
if (!typeFolder.isDirectory()) {
                    if (!typeFolder.mkdirs()) {
                        throw new IOException("Failed to create directory " + typeFolder);
                    }
}
...
String content = XmlPrettyPrinter.prettyPrint(document);
            ...
            if (!blobRootFolder.mkdirs()) {
                throw new IOException("Failed to create directory " + blobRootFolder);
            }
