...
//Synthetic comment -- index 88f28f6..848a4ab 100644
...
//Synthetic comment -- index 693a5f8..05f0fa8 100644
...
//Synthetic comment -- @@ -309,20 +310,42 @@
// - GL version
...
if (minSdkVersion == previousManifest.data.getMinSdkVersion() &&
                        currentSS.hasSameScreenSupportAs(previousSS) &&
...
previousManifest.file.getOsLocation()));
                ...
                // The values are different. Now perform other validity checks.
                // If the only difference is Supports-Screens, then we check for the following:
                // We cannot support a case where two apk declare the following support screen:
                // apk1: small, large
                // apk2: normal.
                // Overlap is considered ok:
                // apk1: small, normal
                // apk2: normal, large
                // this is important only if there no difference in higher priority properties
                // (at this time only minSdkVersion is higher property)
                if (minSdkVersion == previousManifest.data.getMinSdkVersion() &&
                        currentSS.overlapWith(previousSS)) {
...
//Synthetic comment -- index da96cd4..b93216d 100644
...
//Synthetic comment -- @@ -303,14 +308,117 @@
return false;
}
         ...
         * anyDensity), and considers that {@link #overlapWith(SupportsScreens)} returns false.
         * @throws IllegalArgumentException if the two instanced overlap each other.
...
//Synthetic comment -- index c0cb12f..4d6aa76 100644

//Synthetic comment -- @@ -57,4 +57,40 @@
...
assertEquals(Boolean.FALSE, supportsScreens.getLargeScreens());
    ...
    public void testOverlapWith() {
        SupportsScreens supports1 =  SupportsScreens.getDefaultValues(7);
        supports1.setNormalScreens(Boolean.FALSE);

        SupportsScreens supports2 =  SupportsScreens.getDefaultValues(7);
        supports2.setSmallScreens(Boolean.FALSE);
        supports2.setLargeScreens(Boolean.FALSE);

        assertTrue(supports1.overlapWith(supports2));
    }
    ...
    public void testCompareTo() {
        ...
        assertEquals(-1, supportsS.compareTo(supportsN));
        assertEquals( 1, supportsN.compareTo(supportsS));
        assertEquals(-1, supportsN.compareTo(supportsL));
        assertEquals(-1, supportsS.compareTo(supportsL));
        assertEquals(-1, supportsSN.compareTo(supportsNL));
        assertEquals(-1, supportsSN.compareTo(supportsSNL));

        try {
            supportsSL.compareTo(supportsN);
