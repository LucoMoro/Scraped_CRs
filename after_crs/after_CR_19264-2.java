...
//Synthetic comment -- index a6513aa..000bdfa 100755
...
// These are protected by the mTmpValue lock.
    ...
    private final LongSparseArray<SoftReference<Drawable.ConstantState> > mColorDrawableCache
            = new LongSparseArray<SoftReference<Drawable.ConstantState> >();
...
private void clearDrawableCache(
            LongSparseArray<SoftReference<ConstantState>> cache,
int configChanges) {
int N = cache.size();
if (DEBUG_CONFIG) {
...
for (int i=0; i<N; i++) {
            SoftReference<Drawable.ConstantState> ref = cache.valueAt(i);
if (ref != null) {
...
//        + " in " + this + ": " + cs);
if (isColorDrawable) {
                            mColorDrawableCache.put(key, new SoftReference<Drawable.ConstantState>(cs));
} else {
...
private Drawable getCachedDrawable(
            LongSparseArray<SoftReference<ConstantState>> drawableCache,
long key) {
synchronized (mTmpValue) {
            SoftReference<Drawable.ConstantState> ref = drawableCache.get(key);
