...
ArrayList<ApkData> datalist = new ArrayList<ApkData>();
        ArrayList<ManifestData> manifests = new ArrayList<ManifestData>();
...
*/
    private ArrayList<ApkData> checkManifest(IAbstractFile androidManifest,
            ArrayList<ManifestData> manifests) throws ExportException {
try {
...
"Codename in minSdkVersion is not supported by multi-apk export.");
            ...
            System.out.println("\t\t\tGlEsVersion=" + manifestData.getGlEsVersion());
            System.out.println("\t\t\t>>>>>>>>>>>>>>>>>>");

            // compare to other existing manifest.
            for (ManifestData previousData : manifests) {
                ...
                if (minSdkVersion == previousData.getMinSdkVersion() &&
                        compareObjects(manifestData.getSupportsScreens(),
                                previousData.getSupportsScreens()) &&
                        manifestData.getGlEsVersion() == previousData.getGlEsVersion()) {
                    throw new ExportException(String.format(
                            "Android manifests must differ in one of the following attribute:\n" +
                            "- minSdkVersion\n" +
                            "- SupportsScreen\n" +
                            "- GL ES version.\n" +
                            "%1$s and %2$s are considered identical for multi-apk export.",
                            androidManifest.getOsLocation(), "foo"));
                }
            }
...
return mLargeScreens;
        ...
        public boolean equals(Object obj) {
            if (obj instanceof SupportsScreens) {
                ...
                // since all the fields are garanteed to be either Boolean.TRUE or Boolean.FALSE
