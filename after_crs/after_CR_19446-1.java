...
LayoutLibrary layoutLib = targetData.getLayoutLibrary();
            setClippingSupport(layoutLib.getApiLevel() >= 4);
}
...
if (layoutLibrary != null) {
                hasTransparency = layoutLibrary.getApiLevel() >= 5;
}
...
if (log != null) {
     ...
     *
     * @return the API level of the library or -1 if {@link #getStatus()} doesn't return
     * {@link LoadStatus#LOADED}.
     *
     * @see LayoutBridge#getApiLevel()
     */
    public int getApiLevel() {
        if (mBridge != null) {
            return mBridge.getApiLevel();
        } else if (mLegacyBridge != null) {
            return mLegacyBridge.getApiLevel();
        }
...
mStatus = status;
    ...
    public LayoutScene createLegacyScene(SceneParams params) {
