                    ...
                    if (state.isLibrary()) {
                        for (ProjectState projectState : sProjectStateMap.values()) {
                            ...
                            if (libState != null) {
                                ...
                                // This is because in case of a rename of a project, projectClosed
                                // and projectOpened will be called before any other job is run,
                                // so we need to make sure projectOpened is closed with the main
                                // project state up to date.
                                libState.close();
...
onProjectOpened(project, false /*recompile*/);
        ...
        private void onProjectOpened(final IProject openedProject, final boolean recompile) {
            final ProjectState openedState = getProjectState(openedProject);
            if (openedState != null && openedState.hasLibraries()) {

                // create a job
                Job job = new Job("Project Library link update") { //$NON-NLS-1$
                    @Override
                    protected IStatus run(IProgressMonitor monitor) {
                        // reset all linked folders.
                        IStatus status = resetAllProjectLinks(openedState, monitor);
                        ...
                        synchronized (sLock) {
                            for (ProjectState projectState : sProjectStateMap.values()) {
                                if (projectState != openedState) {
                                    ...
                                    if (libState != null) {
                                        foundLibrary = true;
                                        status = linkProjectAndLibrary(openedState,
                                                libState, null, monitor);
                        ...
                        // if the project is a library, then try to see if it's required by
                        // other projects, directly or indirectly
                        if (openedState.isLibrary()) {
                            ...
                            synchronized (sLock) {
                                for (ProjectState projectState : sProjectStateMap.values()) {
                                    if (projectState != openedState &&
                                            projectState.isMissingLibraries()) {

                                        LibraryState libState = projectState.needs(openedState);
                                        if (libState != null) {
                                            status = linkProjectAndLibrary(projectState,
                                                    libState, null, monitor);
                                            if (status.isOK() == false) {
                                                return status;
                                            }
        ...
        public void projectRenamed(final IProject project, final IPath from) {
            ...
            final ProjectState renamedState = getProjectState(project);
                            ...
                            for (ProjectState projectState : sProjectStateMap.values()) {
                                if (projectState != renamedState &&
                                    ...
                                    LibraryState libState = projectState.updateLibrary(
                                            ...
                                            renamedState);
                                    if (libState != null) {
                                        IStatus status = linkProjectAndLibrary(projectState,
                                                libState, from, monitor);
...
/**
     * Remove all linked source folders from the build path and delete the linked folder themselves.
     * @param projectState the project being reset.
     * @return the status
     */
    private IStatus resetAllProjectLinks(ProjectState projectState, IProgressMonitor monitor) {
        try {
            IProject project = projectState.getProject();

            // get the current classpath entries for the project to remove all linked source folders
            // that were added by ADT.
            ...
            // to keep old one--if they link to the same resource, which could have changed)
            ...
            AdtPlugin.logAndPrintError(e, "Library Project", "Failed to reset linked source folder for %1$s: %2$s",
...
*/
    private IStatus linkProjectAndLibrary(
