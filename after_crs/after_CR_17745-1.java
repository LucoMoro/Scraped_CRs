...
*/
    ...
    /**
     * Returns a set of all categories actually used (that is, which have at least
     * one source in them).
     * Might return a empty set, but never returns null.
     */
    public Set<SdkSourceCategory> getCategories() {
        return Collections.unmodifiableSet(mSources.keySet());
    }
     ...
     * The search is O(N), which should be acceptable on the expectedly small source list.
     */
    public boolean hasSource(SdkSourceCategory category, SdkSource source) {
        ArrayList<SdkSource> list = mSources.get(category);
        if (list != null) {
            for (SdkSource s : list) {
                if (s.equals(source)) {
...
/**
 * Interface that a page can optionally implement to be notified when the user
 * switched to that page.
 */
public interface IPageSwitchListener {

    /**
     * Called by the updater window when a page is displayed.
     */
...
public class RemotePackagesPage extends Composite
        implements ISdkListener, IPageSwitchListener {

private final UpdaterData mUpdaterData;

    private boolean mLoadRemoteAddons = true;
...
// Disable the check that prevents subclassing of SWT components
    ...
    public void onPageSwitch() {
        if (mLoadRemoteAddons) {
            // The first time that we switch to this page, try to load the addons list
            // from the remote server.
            if (mUpdaterData.loadRemoteAddonsList()) {
                mLoadRemoteAddons = false;
            }
        }
    }
...
SdkSource source = (SdkSource) c;
                    ...
                    if (mUpdaterData.getSources().hasSource(
...
for (Object c : ((ITreeSelection) sel).toList()) {
if (c instanceof SdkSource &&
                        mUpdaterData.getSources().hasSource(
...
if (parentElement == RepoSourcesAdapter.this) {
                return mUpdaterData.getSources().getAllSources();
...
SdkSources sources = getSources();
        ...
        sources.add(SdkSourceCategory.ADDONS_3RD_PARTY,
                new SdkRepoSource(SdkRepoConstants.URL_GOOGLE_SDK_SITE,
                                  SdkSourceCategory.ADDONS_3RD_PARTY.getUiName()));
...
//Synthetic comment -- @@ -796,4 +799,49 @@
    ...
    /**
     * Loads (or reloads) the remote add-ons list.
     *
     * @return True if succeeded in fetching a remote add-ons list and updating it.
     *   False otherwise (errors are displayed by the task monitor.)
     */
    public boolean loadRemoteAddonsList() {
...
mInternalPageChange = false;
            ...
            if (page instanceof IPageSwitchListener) {
                ((IPageSwitchListener) page).onPageSwitch();
            }
}
}
