...
Logger.getLogger(getClass().getName()).log(Level.SEVERE, "Error encountered", e);
...
if ("DRLCertFactory".equals(f.getProvider().getName()) && certs.isEmpty()) {
fail("should throw when no certificates present");
}
} catch (Exception e) {
throw new Exception("Error generating certificates", e);
}
assertNotNull(certs);
