...
// Retry fetch if validation failed
...
if (usingAlternateXml) {
    // Additional logic for using alternate XML
}

// Fetch the document at the given URL and returns it as a byte array input stream.
...
private String validateXml(ByteArrayInputStream xml, String url, String[] outError, Boolean[] validatorFound) {
    ...
    for (int version = SdkRepository.NS_LATEST_VERSION; version >= 1; version--) {
        try {
            ...
            // Validate against the retrieved XML
            validator.validate(new StreamSource(xml));
            String schemaUri = SdkRepository.getSchemaUri(version);
            if (!xml.equals(getSchemaNamespace(xml, schemaUri))) {
                lastError = "XML does not have the expected namespace.";
                extraError = "Expected namespace: " + schemaUri;
                continue;
            }
            return schemaUri;
...
/**
 * Ensure that the XML root element has the correct namespace.
 */
private String getSchemaNamespace(ByteArrayInputStream xmlStream, String expectedNamespace) {
    // Implementation details to parse XML and validate namespace
}
