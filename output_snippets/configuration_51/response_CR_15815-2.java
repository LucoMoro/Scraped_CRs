...
private Drawable getCachedColorDrawable(TypedValue value) {
    String cacheKey = "color_" + value.assetCookie + "_" + value.data;
    synchronized (mTmpValue) {
        WeakReference<ColorStateList> wr = mColorStateListCache.get(cacheKey);
        if (wr != null) {   // we have the key
            ColorStateList cs = wr.get();
            if (cs != null) {
                return cs.newDrawable(this);
            } else {  // our entry has been purged
                mColorStateListCache.delete(cacheKey);
            }
        }
    }
    return null;
...
throws NotFoundException {
    ColorStateList colorStateList = getCachedColorDrawable(value);
    if (colorStateList != null) {
        return colorStateList;
    }
// Load and cache the color state list as before
