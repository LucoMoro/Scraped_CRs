...
void cleanupDrawableCache(int configChanges) {
...
synchronized (mSync) {
    // Other synchronization logic may go here
}
    ...
    if (cs != null) {
        if (mPreloading) {
            ...
            synchronized (mTmpValue) {
                long colorKey = key | 0x8000000000000000L; // Distinct key for color drawables
                if (!mColorStateListCache.containsKey(key) && !mDrawableCache.containsKey(colorKey)) {
                    mDrawableCache.put(colorKey, new WeakReference<Drawable.ConstantState>(cs));
                }
            }
        }
    }
