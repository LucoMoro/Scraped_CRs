    ...
    while ((providerClassName = Security.getProperty("security.provider." + i++)) != null) {
        if (isValidProviderClass(providerClassName)) {
            try {
                ...
                Logger.getLogger("ProviderLoader").log(Level.SEVERE, "Failed to load provider: " + providerClassName, e); // logging exception
            }
        }
    }
...
private static boolean isValidProviderClass(String providerClassName) {
    ...
    for (String validProvider : validProviders) {
        if (validProvider.equals(providerClassName)) {
            return true;
        }
    }
    return false;
