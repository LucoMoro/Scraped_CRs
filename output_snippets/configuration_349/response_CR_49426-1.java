...
public static final EnumSet<Scope> OTHER_SCOPE = EnumSet.of(Scope.OTHER); // Added Scope.OTHER
}
...
// Added check for OTHER files
for (File otherFolder : project.getOtherFolders()) {
    checkFolder(context, otherFolder);
}

for (File file : project.getOtherFolders()) {
    checkFolder(context, file); // Check files for Scope.OTHER
}
}
...
public boolean appliesTo(@NonNull Context context, @NonNull File file) {
    return super.appliesTo(context, file) || 
           context.getProject().getOtherFolders().stream().anyMatch(folder -> file.getAbsolutePath().startsWith(folder.getAbsolutePath())); // Updated appliesTo for Scope.OTHER
}
