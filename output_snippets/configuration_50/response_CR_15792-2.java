...
/**
 * Executes a remote command on the device, and sends the result to a {@link IShellOutputReceiver}.
 * <p/>The timeout value is used as a maximum waiting time when expecting data from the device.
 * If the command setup or output reception exceeds this time, a {@link TimeoutException} will be thrown.
 *
 ...
 * @throws TimeoutException in case of timeout during command setup or output reception.
 ...
 * receiver's {@link IShellOutputReceiver#isCancelled()} returns <code>true</code>) should be
 * used.
 ...
 * @param receiver the {@link IShellOutputReceiver} that will receive the output of the shell command.
 * @param timeout timeout value in ms for the connection. 0 means no timeout.
 * @throws TimeoutException in case of timeout during execution.
 * @throws IOException in case of I/O error on the connection.
 */
