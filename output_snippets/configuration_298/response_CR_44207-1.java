 ...
 */
public AndroidPackageRenameChange(IFile androidManifest, ITextFileBufferManager manager, IDocument document, Map<String, String> elements, String oldName, String newName, boolean isPackage) {
    super(document);
 ...
 */
public AndroidTypeMoveChange(IFile androidManifest, ITextFileBufferManager manager, IDocument document, Map<String, String> elements, String oldName, String newName) {
 ...
 * @param oldName the old name
 */
public AndroidTypeRenameChange(IFile androidManifest, ITextFileBufferManager manager, IDocument document, Map<String, String> elements, String oldName, String newName) {
    super(document);
    this.mDocument = document;
...
if (mAndroidManifest.exists()) {
    if (mAndroidElements.size() > 0 || mIsPackage) {
        getDocument();
        Change change = new AndroidPackageRenameChange(mAndroidManifest, mManager, mDocument, mAndroidElements, mOldName, mNewName, mIsPackage);
        if (change != null) {
            result.add(change);
        }
        IDocument document;
...
public IDocument getDocument() throws CoreException {
    if (mDocument == null) {
        ...
        mManager.connect(mAndroidManifest.getFullPath(), LocationKind.NORMALIZE, new NullProgressMonitor());
        ITextFileBuffer buffer = mManager.getTextFileBuffer(mAndroidManifest.getFullPath(), LocationKind.NORMALIZE);
...
public class AndroidTypeRenameParticipant extends AndroidRenameParticipant {
    ...
    if (mAndroidManifest.exists()) {
        if (mAndroidElements.size() > 0) {
            getDocument();
            Change change = new AndroidTypeRenameChange(mAndroidManifest, mManager, mDocument, mAndroidElements, mOldName, mNewName);
            if (change != null) {
                result.add(change);
            }
            ...
            if (model != null) {
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    String value = attributes.getNamedItem("class").getValue();
                    if (value != null && value.equals(className)) {
                        AndroidLayoutChangeDescription layoutChange = new AndroidLayoutChangeDescription(className, mLayoutNewName, AndroidLayoutChangeDescription.VIEW_TYPE);
                ...
                try {
                    ...
                    if (mManager != null) {
                        try {
                            mManager.disconnect(mAndroidManifest.getFullPath(), LocationKind.NORMALIZE, new NullProgressMonitor());
...
//<Beginning of snippet n. 7>
                ...
                if (IMarker.PROBLEM, true, IResource.DEPTH_INFINITE) == IMarker.SEVERITY_ERROR) {
