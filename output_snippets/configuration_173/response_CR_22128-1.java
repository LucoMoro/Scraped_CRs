...
private static final long EXPIRATION_VALIDITY = 30 * 365 * 24 * 60 * 60; // 30 years
...
if (loadKeyEntry(osKeyStorePath, storeType) == null) {
// create the store with the key
createNewStore(osKeyStorePath, storeType, output);
}
}

private KeyStore.PrivateKeyEntry loadKeyEntry(String osKeyStorePath, String storeType) throws KeytoolException {
try {
if (mEntry != null) {
return mEntry;
}

throw new KeytoolException("Key entry not found in the keystore.");
} catch (FileNotFoundException e) {
throw new KeytoolException("Keystore file not found: " + osKeyStorePath);
}
}
...
if (KeystoreHelper.createNewStore(osKeyStorePath, storeType, PASSWORD_STRING, DEBUG_ALIAS,
                PASSWORD_STRING, CERTIFICATE_DESC, EXPIRATION_VALIDITY, output)) {
mEntry = loadKeyEntry(osKeyStorePath, storeType);
}
}
