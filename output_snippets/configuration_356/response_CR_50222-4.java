//<Beginning of snippet n. 0>


// The private status of the view that created this player
private IsPrivateBrowsingEnabledGetter mIsPrivateBrowsingEnabledGetter;

private static int IDLE        =  0;
private static int INITIALIZED =  1;
private static int PREPARED    =  2;
private static int STARTED     =  4;
private static int COMPLETE    =  5;
private static int PAUSED      =  6;
private static int USER_PAUSED  =  7;
private static int STOPPED     = -2;
private static int ERROR       = -1;

private int mState = IDLE;

// resume playback
if (mMediaPlayer == null) {
    resetMediaPlayer();
} else if (mState != ERROR && !mMediaPlayer.isPlaying()) {
    mMediaPlayer.start();
    mState = STARTED;
}

switch (audioFocusChange) {
    case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
        // Lost focus for a short time, but we have to check user pause state
        if (mState != ERROR && mState != USER_PAUSED && mMediaPlayer.isPlaying()) pause();
        break;
    case AudioManager.AUDIOFOCUS_GAIN:
        // Regain audio focus
        if (mState == USER_PAUSED) {
            mMediaPlayer.start();
            mState = STARTED;
        } else if (mState != USER_PAUSED && mState != ERROR) {
            // Ensure resuming playback only if not USER_PAUSED and not in error state
            mMediaPlayer.start();
            mState = STARTED;
        }
        break;
}
}

private void pause() {
if (mState == STARTED) {
    if (mTimer != null) {
        mTimer.purge();
    }
    mMediaPlayer.pause();
    mState = USER_PAUSED; // Change to USER_PAUSED instead of PAUSED
}

//<End of snippet n. 0>