...
private static int USER_PAUSED  = 7; // New state for user pausing
...
case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
    if (mState == USER_PAUSED) { 
        // Allow playback to continue if user paused
        break;
    }
...
private void pause() {
    if (mState == STARTED) {
        ...
        mMediaPlayer.pause();
        mState = USER_PAUSED; // Set to USER_PAUSED instead of PAUSED
    }
}

// New method to handle audio focus gain
private void onAudioFocusGained() {
    if (mState == USER_PAUSED) {
        mMediaPlayer.start();
        mState = STARTED; // Resume playback
    }
}
