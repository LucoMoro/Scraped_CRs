...
String density = d.getDensity();
if (resolution == null || resolution.equals("Unknown")) {
    resolution = "Unknown";
}
if (density == null || density.equals("Unknown")) {
    density = "Unknown";
}
String deviceInfo = String.format("%s (Resolution: %s, Density: %s)", name, resolution, density);
...
private void setupDevices(final MenuItem menuBarDevices) {
    ...
    for (String manufacturer : devices.keySet()) {
        ...
        for (final Device d : devices.get(manufacturer)) {
            ...
            deviceItem.setText(String.format("%s (Resolution: %s, Density: %s)", d.getName(), resolution, density));
...
// This contains the Software for the device. Since it has no effect on the emulator whatsoever, we just use a single instance with reasonable
// defaults.
