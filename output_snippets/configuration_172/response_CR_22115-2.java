...
public String getAdapterItemValue(ResourceReference adapterView, ResourceReference itemRef,
    ...
    if (viewClass == null || adapterView == null || itemRef == null || viewRef == null) {
    ...
    switch (viewClass) {
        ...
        case "ImageView":
            return viewRef.getName(); // Handle ImageView case
        case "Button":
            return viewRef.getName(); // Handle Button case
        case "ProgressBar":
            return viewRef.getName(); // Handle ProgressBar case
        case "CheckBox":
            return viewRef.getName(); // Handle CheckBox case
        default:
            throw new UnsupportedOperationException("Unsupported view class: " + viewClass); // Throw exception for unsupported types
    }
}

public AdapterBinding getAdapterBinding(ResourceReference adapterView) {
    if (adapterView == null) {
        throw new IllegalArgumentException("adapterView cannot be null");
    }
    AdapterBinding binding = createAdapterBinding(adapterView); // Logic to create binding
    if (binding == null || !binding.isActive()) {
        binding = new AdapterBinding(); // Consider a new instance if the binding is inactive
    }
    return binding;
}
//<End of snippet n. 0>
