...
for (File f : currDir.listFiles()) {
    if (f.isDirectory()) {
        // Check if it contains either an Android manifest or is a valid 'samples' directory
        File manifestFile = new File(f, SdkConstants.FN_ANDROID_MANIFEST_XML);
        if (manifestFile.isFile() || (f.getName().equals(FD_SAMPLES) && f.listFiles().length == 1 && f.listFiles()[0].isFile() && f.listFiles()[0].getName().equals(SdkConstants.FN_ANDROID_MANIFEST_XML))) {
            try {
...
private void validatePage() {
    ...
    for (File file : topLevelSamplesPath.listFiles()) {
        if (file.isDirectory() && (file.getName().equals(FD_SAMPLES) || file.listFiles().length == 1 && new File(file, SdkConstants.FN_ANDROID_MANIFEST_XML).exists())) {
