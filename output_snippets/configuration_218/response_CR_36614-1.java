...
private void setProxyProperties(Properties props) {
    final String JAVA_PROP_HTTPS_PROXY_HOST = "https.proxyHost";     
    final String JAVA_PROP_HTTPS_PROXY_PORT = "https.proxyPort";     

    String currentProxyHost = props.getProperty(JAVA_PROP_HTTPS_PROXY_HOST);
    String currentProxyPort = props.getProperty(JAVA_PROP_HTTPS_PROXY_PORT);
    ...
    if (proxyHost != null && !proxyHost.isEmpty() && proxyPort != null && !proxyPort.isEmpty()) {
        ...
        if (proxyHost == null || proxyHost.isEmpty()) {
            monitor.logError("Skipping setting proxy property: proxyHost is empty");
        }
        if (proxyPort == null || proxyPort.isEmpty()) {
            monitor.logError("Skipping setting proxy property: proxyPort is empty");
        }
    }

    if (!validProxy && (currentProxyHost == null && currentProxyPort == null)) {
        monitor.logError("Using default proxy settings because no valid proxy information was provided.");
    }
}