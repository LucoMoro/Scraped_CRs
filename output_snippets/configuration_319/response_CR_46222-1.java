...
private void checkFoldersForMixedDrawables(File[] folders, Context context) {
    if (folders != null) {
        ...
        for (File folder : folders) {
            ...
            if (folderName.startsWith(DRAWABLE_FOLDER)) {
                ...
                List<String> conflictingFiles = containsMixedDrawables(folder, context);
                if (!conflictingFiles.isEmpty()) {
                    context.report(ICON_EXTENSION, folder, context.getLocation(folder), "Mixed drawable types found in folder: " + folder.getAbsolutePath() + " Conflicting files: " + String.join(", ", conflictingFiles));
                }
            }
            if (folder.isDirectory()) {
                File[] subFolders = folder.listFiles();
                if (subFolders != null) {
                    checkFoldersForMixedDrawables(subFolders, context);
                }
            }
        }

        if (checkFolders && folderToNames.size() > 0) {
            checkDensities(context, folderToNames);
        }
    }
}

private List<String> containsMixedDrawables(File folder, Context context) {
        ...
        for (File file : files) {
            if (file.isFile()) {
                if (file.getName().endsWith(".png")) {
                    hasPng = true;
                    context.log("Found PNG: " + file.getAbsolutePath());
                    conflictingFiles.add(file.getAbsolutePath());
                } else if (file.getName().endsWith(".xml")) {
                    hasXml = true;
                    context.log("Found XML: " + file.getAbsolutePath());
                    conflictingFiles.add(file.getAbsolutePath());
                }
            }
...
private void checkDensities(Context context, Map<File, Set<String>> folderToNames) {
    // TODO: Implement logic to check densities
}

if (context.isEnabled(ICON_DENSITIES)) {
    // Logic for checking missing densities
}
//<End of snippet n. 2>
