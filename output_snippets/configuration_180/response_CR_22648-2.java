...
if (listFqcn.endsWith(EXPANDABLE_LIST_VIEW)) {
    ...
    binding.addItem(new DataBindingItem(LayoutMetadata.DEFAULT_GRID_ITEM, true /* isFramework */, 1));
} else if (listFqcn.endsWith(LIST_VIEW)) {
...
@Override
public void fillMenu(IMenuManager menu, String selected) {
    // Existing actions
    action = new SetListTypeAction("Multiple Choice List Item", "simple_list_item_multiple_choice", selected);
    new Separator().fill(menu, -1);
    action = new SetListTypeAction("Simple Expandable List Item", "simple_expandable_list_item_1", selected); //$NON-NLS-1$
    new ActionContributionItem(action).fill(menu, -1);
    action = new SetListTypeAction("Simple 2-Line Expandable List Item", "simple_expandable_list_item_2", selected); //$NON-NLS-1$
    new ActionContributionItem(action).fill(menu, -1);

    // New GridView actions
    action = new SetListTypeAction("Simple Grid Item", "simple_grid_item", selected); //$NON-NLS-1$
    new ActionContributionItem(action).fill(menu, -1);
    action = new SetListTypeAction("Simple Grid Item with Header", "simple_grid_item_with_header", selected); //$NON-NLS-1$
    new ActionContributionItem(action).fill(menu, -1);
    action = new SetListTypeAction("Simple Grid Item with Footer", "simple_grid_item_with_footer", selected); //$NON-NLS-1$
    new ActionContributionItem(action).fill(menu, -1);
