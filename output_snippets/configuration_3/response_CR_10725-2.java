...
Set<String> existingRecipients = new HashSet<>();
...
if (!TextUtils.isEmpty(r.name) || Mms.isEmailAddress(r.number)) {
    ...
    String recipientKey = r.name + ":" + r.number;
    if (!existingRecipients.contains(recipientKey)) {
        existingRecipients.add(recipientKey);
        rl.add(r);
    }
}
