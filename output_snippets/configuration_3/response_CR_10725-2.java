...
if (!TextUtils.isEmpty(r.name) && !TextUtils.isEmpty(r.number) && !recipientExists(r, rl)) {
    rl.add(r);
    i += 2; // Updated index logic to advance correctly
}
//<End of snippet n. 1>
...
private boolean recipientExists(Recipient r, List<Recipient> rl) {
    for (Recipient existing : rl) {
        if (existing.number.equals(r.number.replaceAll("[^\\d]", "")) && existing.name.replaceAll("\\s+", "").equalsIgnoreCase(r.name.replaceAll("\\s+", ""))) {
            return true;
        }
    }
    return false;
