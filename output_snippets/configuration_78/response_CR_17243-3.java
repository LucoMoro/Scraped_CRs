...
assertEquals(message, CpuFeatures.ARMEABI, getCpuAbi2());
            ...
            String message = String.format("CPU is not ARM v7 compatible, so Build.CPU_ABI must "
                    + "be %s and Build.CPU_ABI2 must be unset or have a valid value.", CpuFeatures.ARMEABI);
assertEquals(message, CpuFeatures.ARMEABI, Build.CPU_ABI);
            String cpuAbi2Value = getCpuAbi2();
            assertTrue(message, cpuAbi2Value == null || cpuAbi2Value.isEmpty());
}
}

private String getCpuAbi2() {
        String abi2 = SystemProperties.get("ro.product.cpu.abi2", null);
        return abi2 != null && !abi2.equals("unknown") ? abi2 : null;
}
}
