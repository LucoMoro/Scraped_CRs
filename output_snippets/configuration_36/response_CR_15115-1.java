...
if (e.detail == DND.DROP_MOVE) {
    try {
        for (UiElementNode element : mDragElements) {
            if (element != null && canRemoveElement(element)) {
                removeElement(element);
                AdtPlugin.printToConsole("CanvasDND", "Removed element: " + element);
            }
        }
    } catch (Exception ex) {
        AdtPlugin.printToConsole("CanvasDND", "Error removing elements: " + ex.getMessage());
    }
}
...
private boolean canRemoveElement(UiElementNode element) {
    // Implement logic to check if the element can be removed
    return true; // Placeholder
}

private void removeElement(UiElementNode element) {
    // Implement the actual removal logic
}
