...
private boolean isMarkupElement(Element element) {
    ...
    // Recognize multi-character tags such as <b>, <i>, <u>, <strong>, <em>, etc.
    return mStyle == XmlFormatStyle.RESOURCE && (tagName.length() > 1);
}

/**
 * Implement recursive parsing logic for nested HTML elements
 */
private String parseElement(Element element) {
    ...
    for (int i = 0; i < children.getLength(); i++) {
        ...
        if (child instanceof Element) {
            ...
            result.append(parseElement((Element) child));
            result.append("</").append(child.getNodeName()).append(">");
        } else {
