...
public class InstrumentationTest extends InstrumentationTestCase {
    ...
    for (int i = 0; i < keyDownList.size(); i++) {
        assertEquals(sendStringSync("A").charAt(i), keyDownList.get(i).getKeyCode());
    }

    for (int i = 0; i < keyUpList.size(); i++) {
        assertEquals(sendStringSync("A").charAt(i), keyUpList.get(i).getKeyCode());
    }
}
...
assertEquals('A', mKeyCharacterMap.getMatch(sendStringSync("A").charAt(0), chars));
assertEquals('B', mKeyCharacterMap.getMatch(sendStringSync("B").charAt(0), chars));
}
...
assertEquals('A', mKeyCharacterMap.getMatch(sendStringSync("A").charAt(0), chars, 0));
assertEquals('B', mKeyCharacterMap.getMatch(sendStringSync("B").charAt(0), chars, 0));
}
