...
mShell.setMenuBar(menuBar);
        ...
        MenuItem configureDevice = new MenuItem(menuTools, SWT.NONE);
        configureDevice.setText("Configure Device...");
        configureDevice.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                onConfigureDevice();
            }
            ...
            new MenuBarWrapper(APP_NAME_MAC_MENU, menuTools) {
                ...
                public void printError(String format, Object... args) {
                    if (mUpdaterData != null) {
                        mUpdaterData.getSdkLog().error("Error occurred: " + String.format(format, args));
                    }
                }
            };
        } catch (Exception e) {
            mUpdaterData.getSdkLog().error("Failed to setup menu bar: " + e.getMessage());
        }
    }

    private void onConfigureDevice() {
        // Implementation of device configuration as per CR_TASK specifications.
    }
