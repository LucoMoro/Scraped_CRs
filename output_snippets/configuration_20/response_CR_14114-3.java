...
if (duration < WEEK_IN_MILLIS && minResolution < WEEK_IN_MILLIS) {
    ...
    long weekStart = currentTime - WEEK_IN_MILLIS;

    for (long callTime : callLog) { // Assuming callLog is an array or list of timestamps
        if (callTime >= weekStart && callTime < currentTime) {
            int dayOfYear = new java.util.Calendar.getInstance().get(java.util.Calendar.DAY_OF_YEAR);
            uniqueDays.add(dayOfYear);
        }
    }
