...
int messageStatus = getMessageStatus(pdu);
if (isStatusMessage && messageStatus == SUCCESS) {
    MessagingNotification.nonBlockingUpdateNewMessageIndicator(context, true, isStatusMessage);
} else if (messageStatus == PENDING || messageStatus == FAILED) {
    logError("Report generation not allowed for status: " + messageStatus);
}
