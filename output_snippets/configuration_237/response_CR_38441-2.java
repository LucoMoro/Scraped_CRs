...
import java.util.Optional; // Import Optional for improved error handling
...
private static Optional<TimeZone> getCustomTimeZone(String id) {
        if (id == null || id.length() < 7 || !id.matches("^GMT[+\\-]\\d{2}(:\\d{2})?$")) {
        ...
        if (hour < 0 || hour > 14) { // Changed upper limit to 14
            return Optional.empty(); // Return empty for hour out of range
        }
        int index = position[0];
        if (index == -1) {
            return Optional.empty(); // Return empty for invalid format
        }
        ...
        if (index < formattedName.length() && formattedName.charAt(index) == ':') {
            ...
            if (position[0] == -1 || minute < 0 || minute > 59) {
                return Optional.empty(); // Return empty for minute out of range
            }
            raw += minute * 60000;
        ...
        return Optional.of(new SimpleTimeZone(raw, formattedName)); // Return Optional with SimpleTimeZone
    }
