...
if (!mMessages.isEmpty()) {
    try {
        ...
        Log.e("LogFilter", "Error clearing messages", e);
    }
}
mTable.removeAll();
...
try {
    ...
    for (int i = 0; i < totalCount; i++) {
        LogMessage msg;
        try {
            msg = mNewMessages.get(i);
            // process msg
        } catch (IndexOutOfBoundsException e) {
            Log.e("LogFilter", "Index out of bounds while accessing mNewMessages", e);
        }
    }  
} catch (Exception e) {
    Log.e("LogFilter", "Error processing messages", e);
}
