...
private void createMenuBar() {
        ...
        manageSdk.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (hasPermission("manageSdk")) {
                    onSdkManager();
                } else {
                    mUpdaterData.getSdkLog().error("Unauthorized access attempt to SDK Manager");
                }
            }
        });
        ...
        configureDevices.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                if (hasPermission("configureDevices")) {
                    onConfigureDevices();
                } else {
    ...
    private boolean hasPermission(String permission) {
        // Logic to check if the current user has the required permission
        if ("configureDevices".equals(permission)) {
