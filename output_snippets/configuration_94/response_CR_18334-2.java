...
case RIL_UNSOL_EXIT_EMERGENCY_CALLBACK_MODE: 
    if (mIsPhoneInEcmState) {
        ...
        Log.d(LOG_TAG, "Exited Emergency Callback Mode");
        if (mEcmExitRespRegistrant != null) {
            mEcmExitRespRegistrant.notifyRegistrant();
        }
    } else {
        Log.d(LOG_TAG, "Failed to exit Emergency Callback Mode: already not in ECF state.");
    }
    break;
...
if (ar.exception == null) {
    if (mIsPhoneInEcmState) {
        mIsPhoneInEcmState = false;
        setSystemProperty(TelephonyProperties.PROPERTY_INECM_MODE, "false");
        Log.d(LOG_TAG, "Successfully exited Emergency Callback Mode.");
    }
}
break;
