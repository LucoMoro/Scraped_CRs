...
public class DragImageCreator {
    ...
    public void createDragImage(Event e) {
        if (mImage != null && !mIsPlaceholder) {
            ...
            int width = Math.round(data.width * zoomLevel);
            int height = Math.round(data.height * zoomLevel);
    ...
    private float getCurrentZoomLevel() {
        return getCanvasZoomLevel();
    }

    private float getCanvasZoomLevel() {
    ...
    // Add listener for dynamic zoom updates
