...
public class SensorFeatureTest extends AndroidTestCase {
    ...
    public void testGetSensorList() throws Exception {
        ...
        assertFeatureForSensor(featuresLeft, PackageManager.FEATURE_CAMERA, Sensor.TYPE_CAMERA);
        assertFeatureForSensor(featuresLeft, PackageManager.FEATURE_CAMERA_FRONT, Sensor.TYPE_CAMERA); // assuming TYPE_CAMERA as applicable for both
    ...
    private static Set<String> getAllSensorFeatures() throws IllegalArgumentException, IllegalAccessException {
        ...
        sensorFeatures.add(PackageManager.FEATURE_CAMERA);
        sensorFeatures.add(PackageManager.FEATURE_CAMERA_FRONT);
    ...
    private void assertFeatureForSensor(Set<String> featuresLeft, String expectedFeature, int expectedSensorType) {
        ...
        String message = "PackageManager#hasSystemFeature(" + expectedFeature + ") returns "
                + hasSensorFeature + " but SensorManager#getSensorList(" + expectedSensorType + ") shows sensors " + sensorNames;
...
public class TelephonyFeatureTest extends AndroidTestCase {
    ...
    public void testNonSDKFeatures() {
        PackageManager packageManager = getContext().getPackageManager();
        // Confirm non-SDK features do not utilize android namespace
        assertFalse("Non-SDK feature presumed to exist", packageManager.hasSystemFeature("android.custom_feature"));
    }
}
