...
try {
    ...
    if (enableEmergencyAlerts) {
        ...
        enableDisableEmergencyChannels(manager, emergencyIdRange, true, isCdma);
        if (DBG) log("enabled emergency cell broadcast channels");
    } else {
        if (DBG) log("disabling emergency cell broadcast channels");
        enableDisableEmergencyChannels(manager, emergencyIdRange, false, isCdma);
        if (DBG) log("disabled emergency cell broadcast channels");
    }

    if (enableChannel50Alerts) {
        if (!prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CHANNEL_50_ALERTS, false)) {
    ...
    Toast.makeText(this, "Error enabling cell broadcast channels: " + ex.getMessage(), Toast.LENGTH_SHORT).show();
}

private void enableDisableEmergencyChannels(SmsManager manager, String emergencyIdRange, boolean enable, boolean isCdma) {
    if (!TextUtils.isEmpty(emergencyIdRange)) {
        setChannelRange(manager, emergencyIdRange, enable);
    } else if (isCdma) {
        if (enable) {
        ...
        if (enable) {
            ...
            manager.disableCellBroadcast(SmsCbConstants.MESSAGE_ID_CMAS_ALERT_PRESIDENTIAL_LEVEL);
        }
    }
}
//<End of snippet n. 1>
