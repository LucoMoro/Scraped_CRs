...
try {
    ...
    if (enableEmergencyAlerts) {
        ...
        if (!TextUtils.isEmpty(emergencyIdRange)) {
            ...
            manager.enableCellBroadcastRange(SmsEnvelope.SERVICE_CATEGORY_CMAS_PRESIDENTIAL_LEVEL_ALERT,
            ...
            manager.enableCellBroadcastRange(SmsCbConstants.MESSAGE_ID_PWS_FIRST_IDENTIFIER,
        ...
        if (!TextUtils.isEmpty(emergencyIdRange)) {
            ...
            manager.disableCellBroadcastRange(SmsEnvelope.SERVICE_CATEGORY_CMAS_PRESIDENTIAL_LEVEL_ALERT,
                    SmsEnvelope.SERVICE_CATEGORY_CMAS_TEST_MESSAGE);
            manager.enableCellBroadcast(SmsEnvelope.SERVICE_CATEGORY_CMAS_PRESIDENTIAL_LEVEL_ALERT);
        } else {
            manager.disableCellBroadcastRange(SmsCbConstants.MESSAGE_ID_PWS_FIRST_IDENTIFIER,
                    SmsCbConstants.MESSAGE_ID_PWS_LAST_IDENTIFIER);
            manager.enableCellBroadcast(SmsCbConstants.MESSAGE_ID_CMAS_ALERT_PRESIDENTIAL_LEVEL);
        }
...
if (enableChannel50Alerts) {
    enableChannel50Alerts.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
        @Override
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            try {
                // Trigger update for broadcast messages based on preference change
                updateBroadcastMessages((Boolean) newValue);
                return true;
            } catch (Exception e) {
                Log.e(TAG, "Failed to update broadcast messages", e);
                return false;
            }
        }
    });
