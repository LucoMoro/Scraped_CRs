...
public class AsecTests extends AndroidTestCase {
    ...
    public void testCreateContainer() {
        try {
            ...
            Assert.assertEquals(StorageResultCode.OperationSucceeded,
                mConnector.doCommand(String.format(Locale.getDefault(), "create %d %d %s %s %d", id, sizeMb, fstype, key, ownerUid)));
        } catch (Exception e) {
    ...
    public void testCreateMinSizeContainer() {
        try {
            ...
            Assert.assertEquals(StorageResultCode.OperationSucceeded,
                mConnector.doCommand(String.format(Locale.getDefault(), "createMinSize %d %d %s %s %d", id, sizeMb, fstype, key, ownerUid)));
        } catch (Exception e) {
            failStr(e.getMessage());
        }
    }
...
String cmd = String.format(Locale.getDefault(), "asec create %d %d %s %s %d", id, sizeMb, fstype, key, ownerUid);
if (!isValidSize(sizeMb) || !isValidOwnerUid(ownerUid)) {
