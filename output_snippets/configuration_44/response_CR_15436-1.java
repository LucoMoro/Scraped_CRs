...
private void saveState() {
    selectedNodes.clear();
    expandedNodes.clear();
    if (mOutline1.getControl() instanceof Tree) {
        Tree tree = (Tree) mOutline1.getControl();
...
private void recordState(TreeItem item) {
    if (item.getItemCount() > 0) {
        expandedNodes.add(item.getData());
        for (TreeItem child : item.getItems()) {
            recordState(child);
        }
    }
    if (item.getParent() != null) {
        selectedNodes.add(item.getData());
    }
}
...
private void restoreItemState(TreeItem item) {
    if (expandedNodes.contains(item.getData())) {
        item.setExpanded(true);
    }
    if (selectedNodes.contains(item.getData())) {
        mOutline1.getTreeViewer().setSelection(new TreeSelection(item.getParent().getData()), true);
    }
}
//<End of snippet n. 0>
