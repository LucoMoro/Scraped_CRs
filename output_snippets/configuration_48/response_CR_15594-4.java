...
private static final int SCO_CONNECTED = 1;
private static final int SCO_DISCONNECTED = 0;
...
private void resetAtState() {
    switch (mPhoneState) {
        ...
        case DIALING:
            callsetup = 2;
            mAudioPossible = true;
            callStarted(); // Call started indication
            break;
        case INCOMING:
        case WAITING:
            callsetup = 1;
...
private void handleScoConnection(Message msg, BluetoothDevice device) {
    if (msg.arg1 == ScoSocket.STATE_CONNECTED) {
        Log.i(TAG, "Routing audio for incoming SCO connection");
        mConnectedSco = (ScoSocket) msg.obj;
        mAudioManager.setBluetoothScoOn(true);
        broadcastAudioStateIntent(BluetoothHeadset.AUDIO_STATE_CONNECTED, device);
    } else if (msg.arg1 == ScoSocket.STATE_DISCONNECTED) {
        ...
        if (args.length >= 1 && args[0].equals(1)) {
            synchronized (BluetoothHandsfree.this) {
                if (!mWaitingForVoiceRecognition) {
                    // Implement voice recognition handling
                }
            }
        }
    }
