...
if (args.length >= 5 && args[0].equals("ping")) {
            stats.ping(args[1], args[2], args[3], args[4]);
return;
...
*
     ...
     * @param glVersion OpenGL version to report
     * @param glRenderer OpenGL renderer to report
*/
public void ping(String app, String version, String glVersion, String glRenderer) {
        doPing(app, version, glVersion, glRenderer);
}
...
*
     ...
     * @param glRenderer OpenGL renderer to report
*/
private void doPing(final String app, String version, String glVersion, String glRenderer) {
    // Validate the application, version, OpenGL version, and renderer input.
    final String normalVersion = normalizeVersion(app, version);
    if (glVersion == null || glRenderer == null) {
        throw new IllegalArgumentException("OpenGL version or renderer cannot be null");
    }
    ...
    // Run the actual ping in a background thread.
    new Thread(new Runnable() {
        @Override
        public void run() {
            try {
                actuallySendPing(app, normalVersion, glVersion, glRenderer, id);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
...
*/
private void actuallySendPing(String app, String version, String glVersion, String glRenderer, long id)
        throws IOException {
    ...
    String encodedGlVersion = URLEncoder.encode(glVersion, "UTF-8");
    String encodedGlRenderer = URLEncoder.encode(glRenderer, "UTF-8");
    ...
    URL url = new URL(
        ...
        "/service/update?as=androidsdk_" + app +        //$NON-NLS-1$
            ...
            "&glv=" + encodedGlVersion +                //$NON-NLS-1$
            "&glr=" + encodedGlRenderer);               //$NON-NLS-1$
