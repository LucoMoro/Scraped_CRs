...
// the application name and version number, and Android GL info must also be supplied
if (args.length >= 4 && args[0].equals("ping")) {
    stats.ping(args[1], args[2], args[3]);
    return;
 ...
 * @param androidGlInfo Android GL information to report
 */
public void ping(String app, String version, String androidGlInfo) {
    doPing(app, version, androidGlInfo);
}
 ...
 * @param androidGlInfo Android GL information to report
 */
private void doPing(final String app, String version, String androidGlInfo) {
    ...
    if (!isUserAuthorizedForGLInfo()) {
        throw new SecurityException("User not authorized to share Android GL info.");
    }

    new Thread(() -> {
        try {
            actuallySendPing(app, normalVersion, androidGlInfo, id);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }).start();
 ...
 * @param androidGlInfo Android GL information
 * @param id of the local installation
 * @throws IOException if the ping failed
 */
private void actuallySendPing(String app, String version, String androidGlInfo, long id)
        throws IOException {
    ...
    String glInfo  = URLEncoder.encode(androidGlInfo, "UTF-8");
