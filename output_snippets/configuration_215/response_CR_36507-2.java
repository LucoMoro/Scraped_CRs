...
if (args.length >= 3 && args[0].equals("ping") && mStore.isPingOptIn()) {
    stats.ping(args[1], args[2]);
    return;
} else if (args.length > 0) {
    Log.w("ddms", "Unknown argument: " + args[0]);
}
//<End of snippet n. 0>
    ...
    new Thread(new Runnable() {
        ...
        public void run() {
            try {
                ...
                Log.w("ddms", "Ping failed due to an IO exception");
            }
        }
    }).start();
}
...
private void actuallySendPing(String app, String version, long id) throws IOException {
    ...
    if (conn.getResponseCode() != HttpURLConnection.HTTP_OK &&
        ...
        throw new IOException("HTTP response code: " + conn.getResponseCode() + ", Message: " + conn.getResponseMessage());
    }
}
...
private final Map<String, String> mEnvVars = new HashMap<>();
