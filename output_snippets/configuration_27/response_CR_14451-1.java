...
public void parseArgs(String[] args) {
    ...
    if (verb == null) {
        needsHelp = String.format("Expected verb after global parameters but found '%1$s' instead.", a);
        return;
    }

    // Error if it was not a valid object for that verb
    if (directObject == null) {
        needsHelp = String.format("Expected object for verb '%1$s' but found none.", verb);
        return;
    }
    ...
    if (missing != null) {
        needsHelp = String.format("The %1$s %2$s must be defined for action '%3$s %4$s'", plural ? "parameters" : "parameter", missing);
    }
