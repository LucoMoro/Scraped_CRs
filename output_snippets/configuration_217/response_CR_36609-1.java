...
private static final Set<String> benignFileTypes = Set.of(
        ".git", ".bak", ".tmp", ".txt", ".xml~", ".log", ".csv", ".json", ".orig", ".swp", ".*");

private String getFileType(String location) {
    if (location == null || location.isEmpty()) {
        throw new IllegalArgumentException("Invalid file location"); // Throw exception for invalid location
    }
...
if (m.matches()) {
    ...
    if (!benignFileTypes.contains(fileType)) {
        ...
        logBenignFileSkip(location); // Add logging for benign file skips
    }
//<End of snippet n. 0>