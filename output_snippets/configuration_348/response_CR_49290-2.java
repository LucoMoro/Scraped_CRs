...
copyButton.addListener(SWT.Selection, event -> {
    ...
    try {
        Control[] selectedControls = mPackageRootComposite.getChildren(); // Replace with actual selection logic related to user selection
...
printButton.addListener(SWT.Selection, event -> {
    ...
    if (printerData != null) {
        ...
        try {
            ...
            gc.drawText("Content to print", 10, 10);
            gc.dispose();
            printer.end();
        } catch (Exception e) {
            // Handle error
